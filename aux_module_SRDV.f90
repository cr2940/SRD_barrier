...
contains
  ...
  !  end subroutine
    ...
    subroutine small_cells_update_correct(ii,jj,dtdx,qold,qold2,qnew,qnew2,aux,&              !  [   ]
      ...
      ! local
      real(8) :: n_vec(2),t_vec(2),x,y,n_vec2(2),t_vec2(2),vec1(2),vec2(2),vec3(3),vec4(3)
      real(8) :: vec5(2),vec6(2)
      real(8) :: q_hbox_d(3,mx*4), q_hbox_u(3,mx*4), aux_hbox_u(mx*4),aux_hbox_d(mx*4)
      logical :: L2R, R2L
      integer :: is,js,i,j,k,m,m2,num_frags_d(mx*4),index_frags_d(2,4,mx*4)
      integer :: num_frags_d2(mx*4),index_frags_d2(2,4,mx*4),coef
      integer :: num_frags_u(mx*4),index_frags_u(2,4,mx*4),look(2),ll,mm
      integer :: num_frags_u2(mx*4),index_frags_u2(2,4,mx*4)
      real(8) :: hbox_areas_u(mx*4), area_frags_u(4,mx*4)
      real(8) :: hbox_areas_d(mx*4), area_frags_d(4,mx*4)
      real(8) :: hbox_areas_u2(mx*4), area_frags_u2(4,mx*4)
      real(8) :: hbox_areas_d2(mx*4), area_frags_d2(4,mx*4)
...
!  If you use vec2/vec3 to rotate, use -n_vec2/-t_vec2 to revert back
 ...
 ! x - swipe
          select case (type_supper(i))  ! turn all the rotate into normal state into negative coef for RF
          case (1) ! TYPE 1 Cut cell
            ...
            call rotate_state(qnew2(:,is,js),qnew2(:,is,js),coef*vec1,coef*vec2) ! negate these for ot only here
...
! if not, get rid of second negatives
